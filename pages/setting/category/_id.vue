    ...
    <div class="content">
      ...
      <div class="panel col-lg-9">
        <div class="panel-heading customPanelHeading">
          <h5 class="panel-title">{{$t("setting.category")}}</h5>
          <p class="content-group-lg">{{$t("setting.categoryDes")}}</p>
        </div>
        <div class="panel body panel-no-border">
          <div class="panel-body">
            <div class="col-md-4" style="padding-left: 0px !important">
              <div class="form-group has-feedback">
                <input class="form-control" v-model="description" v-on:keyup.enter="search()" :placeholder="$t('setting.categorySearchDes')">
            ...
            <div class="pt-20">
              <fieldset class="content-group" style="margin-top:40px">
                <div class="row">
                  <div class="col-xs-4">
                    <input type="text" v-if="!formData.dataKey" name="categoryId" :data-vv-as="$t('setting.categoryId')" :placeholder="$t('setting.categoryId')" v-validate="{ required: true, regex: /^[a-zA-Z0-9\-\_]+$/ }" class="form-control" v-model="formData.id">
                ...
                <table class="table">
                  ...
                  <tbody>
                    <tr v-for="(item, i) in categoryData.data" :key="i" :value="item">
                      ...
                      <td>{{item.name}}</td>
                      <td>
                        <div :title="$t('setting.updateCategory')" class="display-inline pointer">
              ...
              <el-pagination
                ...
                background
                :current-page.sync="searchCategory.currentPage"
                :page-sizes="[10, 20, 50]"
                :page-size="searchCategory.pageSize"
                layout="sizes, prev, pager, next , jumper"
                :total="categoryData.total">
            </el-pagination>
            <div class="total-list">
              <div v-if="!categoryData.total">{{$t('noData')}}</div>
              <div v-else>{{$t('have')}} {{categoryData.total}} {{$t('row')}}</div>
            </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  middleware: 'authenticated',
	...
	computed: {
		...mapGetters({
			categoryData: 'setting/getCategory'
		}),
		...mapState({
      searchCategory: state => state.setting.searchCategory
		})
  },
  watch: {
  },
    ...
    if (!nuxtState || !nuxtState.state.roleCurrentUser || nuxtState.state.roleCurrentUser != 'admin') {
      ...
      await firebase.database().ref('/category/').on('value', function(data) {
        var category = {data: [], total: 0};
        const obj = data.val();
        for (let key in obj) {
          category.total++;
          category.data.push({
    ...
    handleSizeChange : _.debounce( async function(val) {
      var _this = this;
      _this.searchCategory.pageSize = val;
		},500 ),
		handleCurrentChange: _.debounce(async function (val) {
      var _this = this;
      _this.searchCategory.currentPage = val;
		},500),
		search: _.debounce(async function() {
			var _this = this;
			_this.searchCategory.description = _this.description;
		}, 500),
		validateBeforeSubmit: _.debounce(async function() {
			...
			await _this.$validator.validateAll().then(async result => {
				if (result) {
					try {
						var response = await _this.$store.dispatch('setting/editCategory', _this.formData);
						_this.$notify({
